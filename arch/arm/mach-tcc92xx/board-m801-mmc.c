...
int m801_mmc_init(struct device *dev, int id)
{
	BUG_ON(id > 1);
...
//	printk("%s\n", __func__);
	...
	if(machine_is_m801())
	{
		BITCSET(HwGPIOF->GPFN1, 0x0000F000, 0 ); 
		if(id==0)
		{
//			printk(" resest wifi \n");
			BITCLR(HwGPIOF->GPDAT, Hw11);
			msleep(100);
			BITSET(HwGPIOF->GPDAT, Hw11);
		}
	}
...
int m801_mmc_card_detect(struct device *dev, int id)
{
	BUG_ON(id > 1);
...
int m801_mmc_suspend(struct device *dev, int id)
{
...
int m801_mmc_resume(struct device *dev, int id)
{
	struct tcc_mmc_platform_data *mmc = dev->platform_data;
...
int m801_mmc_set_power(struct device *dev, int id, int on)
{
	if (on) {
		/* power */
		if (id == 1) {
...
int m801_mmc_set_bus_width(struct device *dev, int id, int width)
{
	return 0;
}

struct tcc_mmc_platform_data m801_mmc_platform_data[] = {
	[0] = {
		.slot	= 3,
		...
		.cd_int_num =-1,
		.init	= m801_mmc_init,
		.card_detect = m801_mmc_card_detect,
		.suspend = m801_mmc_suspend,
		.resume	= m801_mmc_resume,
		.set_power = m801_mmc_set_power,
		.set_bus_width = m801_mmc_set_bus_width,
	},	
	[1] = {
		.slot	= 1,
		.caps	= MMC_CAP_SDIO_IRQ | MMC_CAP_4_BIT_DATA
...
static int __init m801_init_mmc(void)
{
	if (!machine_is_m801())
		return 0;
	...
	tcc_mmc_core0_device.dev.platform_data = &m801_mmc_platform_data[0];
	platform_device_register(&tcc_mmc_core0_device);
#endif
#if defined(CONFIG_MMC_TCC_SDHC_CORE1)
	tcc_mmc_core1_device.dev.platform_data = &m801_mmc_platform_data[1];
	platform_device_register(&tcc_mmc_core1_device);
#endif
#endif
	return 0;
}
device_initcall(m801_init_mmc);
