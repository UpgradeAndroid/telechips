...
static unsigned long adc_data[8];
...
extern int tcc_adc_touch_status(void);
extern int tcc_adc_battery_standby(void);
#endif
               ...
               unsigned int channel, unsigned int nr_samples)
{
	...
	if(tcc_adc_battery_standby()){
	    if(tcc_adc_touch_status())
...
static irqreturn_t tcc_adc_irq(int irq, void *pw)
{
	struct adc_device *adc = pw;
	struct tcc_adc_client *client = adc->cur;
	unsigned long flags;
...
static int tcc_adc_probe(struct platform_device *pdev)
{
...
#if 0
	adc->irq = platform_get_irq(pdev, INT_TSADC);
	if (adc->irq <= 0) {
	...
	ret = request_irq(adc->irq, tcc_adc_irq, 0, dev_name(dev), adc);
	if (ret < 0) {
	 dev_err(dev, "failed to attach adc irq\n");
	 goto err_alloc;
	}

	adc->clk = clk_get(NULL, "tcc9200-adc");
	if (IS_ERR(adc->clk)) {
