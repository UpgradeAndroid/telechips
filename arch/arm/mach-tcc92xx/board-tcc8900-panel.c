/* linux/arch/arm/mach-tcc92xx/board-tcc8900-panel.c
 *
...
static void tcc8900_brightness_set(struct led_classdev *led_cdev, enum led_brightness value)
{
...
static struct led_classdev tcc8900_backlight_led = {
	.name		= "lcd-backlight",
	.brightness	= DEFAULT_BACKLIGHT_BRIGHTNESS,
	.brightness_set	= tcc8900_brightness_set,
};

static int tcc8900_backlight_probe(struct platform_device *pdev)
{
	...
	return led_classdev_register(&pdev->dev, &tcc8900_backlight_led);
}

static int tcc8900_backlight_remove(struct platform_device *pdev)
{
	led_classdev_unregister(&tcc8900_backlight_led);
	return 0;
}

static struct platform_driver tcc8900_backlight_driver = {
	.probe	= tcc8900_backlight_probe,
	.remove	= tcc8900_backlight_remove,
	.driver	= {
		.name	= "tcc8900-backlight",
		.owner	= THIS_MODULE,
	},
};

static struct platform_device tcc8900_backlight = {
	.name = "tcc8900-backlight",
};

int __init tcc8900_init_panel(void)
{
	int ret;
	...
	printk("tcc8900_init_panel panel:%d \n",tcc_panel_id);
	...
	platform_device_register(&tcc8900_backlight);
	ret = platform_driver_register(&tcc8900_backlight_driver);
	if (ret)
		return ret;

	return 0;
}
device_initcall(tcc8900_init_panel);
