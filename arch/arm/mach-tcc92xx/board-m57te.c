/* linux/arch/arm/mach-tcc92x/board-m57te.c
 *
...
#if defined(CONFIG_TOUCHSCREEN_TCC_CYPRESS)
...
static struct spi_board_info m57te_spi0_board_info[] = {
	{
		.modalias = "spidev",
		.bus_num = 0,
		.chip_select = 0,
...
static void m57te_nand_init(void)
{
...
static int m57te_nand_ready(void)
{
...
static struct tcc_nand_platform_data tcc_nand_platdata = {
	...
	.init		= m57te_nand_init,
	.ready		= m57te_nand_ready,
};
...
#ifdef CONFIG_TOUCHSCREEN_TCC_CYPRESS
static int tcc_ts_cypress_power(int on)
{
	return 0;
}

static int tcc_ts_cypress_init(void)
{
	return 0;
}

static int tcc_ts_cypress_irq(int en)
{
	if (en) {
	}
	else {
...
static struct tcc_ts_cypress_platform_data tcc_ts_cypress_data = {
	.power	= tcc_ts_cypress_power,
	.init	= tcc_ts_cypress_init,
	.irq	= tcc_ts_cypress_irq,
};
#endif

#if defined(CONFIG_I2C_TCC92XX)
static struct tcc_i2c_platform_data m57te_i2c_platform_data = {
...
static struct tcc_i2c_platform_data m57te_smu_platform_data = {
    .core_clk_name  = "smu",
    .smu_i2c_flag   = 1,
};
#endif
...
#ifdef CONFIG_TOUCHSCREEN_TCC_CYPRESS
	{
		I2C_BOARD_INFO(TCC_TS_CYPRESS_NAME, 0x4d),
	.platform_data = &tcc_ts_cypress_data,
	},
#endif
#ifdef CONFIG_TOUCHSCREEN_TCC_GOODIX
	{
...
static struct platform_device *m57te_devices[] __initdata = {
#if defined(CONFIG_I2C_TCC92XX)
	&tcc9200_i2c_device,
	&tcc9200_smu_device,
#endif
...
static void __init tcc9200_init_machine(void)
{
	...
	spi_register_board_info(m57te_spi0_board_info, ARRAY_SIZE(m57te_spi0_board_info));
...
#if defined(CONFIG_I2C_TCC92XX)
    platform_device_add_data(&tcc9200_i2c_device, &m57te_i2c_platform_data, sizeof(m57te_i2c_platform_data));
    platform_device_add_data(&tcc9200_smu_device, &m57te_smu_platform_data, sizeof(m57te_smu_platform_data));
#endif
	tcc_add_nand_device();

	platform_add_devices(m57te_devices, ARRAY_SIZE(m57te_devices));
}
