...
static int pclk_set_rate(struct clk *clk, unsigned long rate)
{
	...
	if(clk->clock_idx == PERI_HDMIA /*|| clk->clock_idx == PERI_ADC*/ || clk->clock_idx == PERI_SPDIF || clk->clock_idx == PERI_AUD ||
	   clk->clock_idx == PERI_DAI0 || clk->clock_idx == PERI_DAI1 || clk->clock_idx == PERI_DAI2) {
		if (clkrate < 65536)
			dco_shift = 0;
	...
	else if (clk->clock_idx == PERI_I2C0 || clk->clock_idx == PERI_ADC || clk->clock_idx == PERI_I2C1) {
		clksrc = PCDIRECTXIN;
		searchsrc = 0;
...
static int _clk_set_rate(struct clk *clk, unsigned long rate)
{
	...
	if (strcmp(clk->name, "membus") == 0) 
		is_memclk = 1;
	else 
		is_memclk = 0;

	if (is_memclk) spin_lock(&clock_lock); 
	else
		spin_lock_irqsave(&clock_lock, flags);
	...
	if (is_memclk) spin_unlock(&clock_lock); 
	else spin_unlock_irqrestore(&clock_lock, flags);
...
{
	seq_printf(m, "cpu         : %4d MHz\n", tca_ckc_getcpu()/10000);
	seq_printf(m, "display bus : %4d MHz\n", tca_ckc_getfbusctrl(CLKCTRL1)/10000);
//	seq_printf(m, "memory bus  : %4ld MHz\n", clk_get_rate(&clk_membus)/1000000);
	seq_printf(m, "memory bus  : %4d MHz\n", tca_ckc_getfbusctrl(CLKCTRL2)/10000);
	seq_printf(m, "graphic bus : %4d MHz\n", tca_ckc_getfbusctrl(CLKCTRL3)/10000);
	seq_printf(m, "i/o bus     : %4d MHz\n", tca_ckc_getfbusctrl(CLKCTRL4)/10000);
	seq_printf(m, "video bus   : %4d MHz\n", tca_ckc_getfbusctrl(CLKCTRL5)/10000);
	seq_printf(m, "video codec : %4d MHz\n", tca_ckc_getfbusctrl(CLKCTRL6)/10000);
	seq_printf(m, "smu bus     : %4d MHz\n", tca_ckc_getfbusctrl(CLKCTRL7)/10000);
	seq_printf(m, "hsio bus    : %4d MHz\n", tca_ckc_getfbusctrl(CLKCTRL8)/10000);
	seq_printf(m, "camera bus  : %4d MHz\n", tca_ckc_getfbusctrl(CLKCTRL9)/10000);
	return 0;
}
