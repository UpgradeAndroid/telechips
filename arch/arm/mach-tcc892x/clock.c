...
//	clk->real_rate = tca_ckc_setfbusctrl(clk->clock_idx, NOCHANGE, rate/100) * 100;
	...
	if (clk->real_rate != (clkrate*100)) {
		tca_ckc_setfbusctrl(clk->clock_idx, NOCHANGE, clkrate, clksrc);
...
static int pclk_set_rate(struct clk *clk, unsigned long rate)
{
	...
	/* PCK_YYY (n=18,25,26,27,28,29,30,33) */
	if(clk->clock_idx == PERI_HDMIA || clk->clock_idx == PERI_ADMA1 || clk->clock_idx == PERI_ADAM1 || clk->clock_idx == PERI_SPDIF1 ||
	   clk->clock_idx == PERI_ADMA0 || clk->clock_idx == PERI_ADAM0 || clk->clock_idx == PERI_SPDIF0 /*|| clk->clock_idx == PERI_ADC*/) {
		if (clkrate < 65536)
			dco_shift = 0;
