	...
	if(machine_is_tcc9300cm())
	{
		...
		switch(pwr)
		{
			case TCC_HDMI_PWR_INIT:
				...
				gpio_request(TCC_GPEXT2(14), "hdmi_v5p");
				gpio_request(TCC_GPEXT1(7), "hdmi_on");
				gpio_direction_output(TCC_GPEXT2(14), 1);
			...
			case TCC_HDMI_PWR_ON:
				...
				gpio_set_value(TCC_GPEXT2(14), 1);
				gpio_set_value(TCC_GPEXT1(7), 1);
				break;

			case TCC_HDMI_PWR_OFF:
				gpio_set_value(TCC_GPEXT3(2), 0);
				gpio_set_value(TCC_GPEXT2(14), 0);
				gpio_set_value(TCC_GPEXT1(7), 0);
				break;
		}
	}
	else {
