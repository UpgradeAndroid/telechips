...
typedef struct{
	int (*Lcd_Module_probe)(void);
	int (*Lcd_Initailize)(char LCDC_num);
	int (*Lcd_on)(char LCDC_num);
	int (*Lcd_off)(void);
	int	(*Backligth_ctrl)(unsigned int val);
	int (*Lcd_GetModuleSpec)(LCD_MODULE_SPEC_TYPE *val);
}LCD_MODULE_FUNC_TYPE;
