...
int tcc_hdmi_power(struct device *dev, tcc_hdmi_power_s pwr)
{
	...
	if (machine_is_tcc9300st()) {
		printk("%s  tcc9300st  pwr:%d \n", __func__, pwr);
		switch(pwr)
		{
			case TCC_HDMI_PWR_INIT:
				gpio_request(TCC_GPG(13), "hdmi_on");				
				BITCSET(pGPIOG->GPFN1, HwPORTCFG_GPFN5_MASK, HwPORTCFG_GPFN5(0));	//HDMI_ON
				gpio_direction_output(TCC_GPG(13), 1);
				break;

			case TCC_HDMI_PWR_ON:		
				gpio_set_value(TCC_GPG(13), 1);
				break;

			case TCC_HDMI_PWR_OFF:
				gpio_set_value(TCC_GPG(13), 0);	
				break;
		}		
	}
	else {
