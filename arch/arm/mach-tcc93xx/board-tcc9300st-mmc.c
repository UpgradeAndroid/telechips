...
static struct mmc_port_config mmc_ports[] = {
	...
	[1] = {
		...
		.cmd	= TCC_GPE(7),
		.clk	= TCC_GPE(6),
		.cd	= TCC_GPE(23),  // define unused port
...
int tcc9300_mmc_init(struct device *dev, int id)
{
	...
	if(id==1)
	{
		gpio_request(TCC_GPE(0), NULL);
		gpio_request(TCC_GPE(1), NULL);
		gpio_request(TCC_GPE(2), NULL);
		gpio_request(TCC_GPE(3), NULL);

		gpio_request(TCC_GPE(6), NULL);
		gpio_request(TCC_GPE(7), NULL);
		//gpio_request(TCC_GPA(6), NULL);

		tcc_gpio_config(mmc_ports[id].data0, GPIO_FN(5) | GPIO_CD(0));
		tcc_gpio_config(mmc_ports[id].data1, GPIO_FN(5) | GPIO_CD(0));
		tcc_gpio_config(mmc_ports[id].data2, GPIO_FN(5) | GPIO_CD(0));
		tcc_gpio_config(mmc_ports[id].data3, GPIO_FN(5) | GPIO_CD(0));

		tcc_gpio_config(mmc_ports[id].cmd, GPIO_FN(5));
		tcc_gpio_config(mmc_ports[id].clk, GPIO_FN(5) | GPIO_CD(3));
	}
	else
	{
...
struct tcc_mmc_platform_data tcc9300_mmc_platform_data[] = {
	...
	[1] = {
		...
		.cd_int_num = -1,		//HwINT0_EI9,
		.cd_irq_num = -1,		//INT_EI9,
		.peri_name = PERI_SDMMC3,	//PERI_SDMMC1,
		.io_name = RB_SDMMC3CONTROLLER,	//RB_SDMMC1CONTROLLER,
		.pic = HwINT1_SD3,	//HwINT1_SD1,
	},
};
