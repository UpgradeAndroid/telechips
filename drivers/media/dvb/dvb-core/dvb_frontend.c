...
static int gTSIFDataReadSize = TSIF_INT_PACKETCOUNT * TSIF_PACKETSIZE;
...
void TCC_TSIF_ShowPIDTable(void)
{
	...
	printk("PID TABLE ENTRY (%d ENTRIES).\n", pid_param.valid_data_cnt);
}

void TCC_TSIF_Open(int packet_intr_cnt, int packet_dma_cnt ,int mode, int dma_mode)	
{
	int	ret;
	int	handle;
	unsigned int i;
...
int TCC_TSIF_DxBPower(struct file *file, int on_off)
{
	...
	if( 0 != fe->ops.tsif_ioctl(file, IOCTL_TSIF_DXB_POWER , &on_off)) {
		printk("DXB Power Off Error\n");
	}
	return 0;
}

int	TCC_TSIF_ClearPIDTable(struct file *file, int PID)	
{
	struct dvb_device *dvbdev = file->private_data;
	struct dvb_frontend *fe = dvbdev->priv;
	struct dvb_frontend_private *fepriv = fe->frontend_priv;
	...
	if((err = fe->ops.tsif_ioctl(file, IOCTL_TSIF_DMA_STOP, &param))!=-1) {
		pid_param.valid_data_cnt =0 ;
		pid_param.pid_data[pid_param.valid_data_cnt] = PID;
		if (fe->ops.tsif_ioctl(file, IOCTL_TSIF_SET_PID, &pid_param) != -1) {
			if (fe->ops.tsif_ioctl(file, IOCTL_TSIF_DMA_START, &param) != -1) {
				TCC_TSIF_ShowPIDTable();
				return pid_param.valid_data_cnt;
			} else {
				printk("error IOCTL_TSIF_DMA_START ioctl !!!\n");
...
int	TCC_TSIF_SetPID(struct file *file, int PID)	
{
	...
	if(pid_param.valid_data_cnt>(MAX_PID_TABLE_CNT-1)) {
		printk("check pid_param.valid_data_cnt = %d \n", pid_param.valid_data_cnt);
		printk("MAX pid_param.valid_data_cnt = 32 \n");
		printk("PID setting error\n");
		return 0;
	}
	...
	for( i=0; i<pid_param.valid_data_cnt; i++) {
		if( pid_param.pid_data[i] == PID ) {
			printk("We already have 0x%X PID.\n", PID);
			return 0;
		}
	}
...
int	TCC_TSIF_SetPCRPID(struct file *file, unsigned int uiPID)
{
	struct dvb_device *dvbdev = file->private_data;
	struct dvb_frontend *fe = dvbdev->priv;
	struct dvb_frontend_private *fepriv = fe->frontend_priv;
	...
	ret = fe->ops.tsif_ioctl(file, IOCTL_TSIF_SET_PCRPID, &uiPID);
	return ret;
}

unsigned int TCC_TSIF_GetSTC(struct file *file)
{
	struct dvb_device *dvbdev = file->private_data;
	struct dvb_frontend *fe = dvbdev->priv;
	struct dvb_frontend_private *fepriv = fe->frontend_priv;
	...
	fe->ops.tsif_ioctl(file, IOCTL_TSIF_GET_STC, &uiSTC);
	return uiSTC;
}
			...
			unsigned int cmd, void *parg)
{
	...
	if (fe->ops.tsif_ioctl) {
		switch(cmd) {
			case FE_DXB_POWER_CMD:
				{
					int on_off = (int)parg;
					err = TCC_TSIF_DxBPower(file, on_off);
				}
				break;
			case FE_DXB_CLEAR_TABLE_CMD:
				{
					int PID = (int)parg;
					err = TCC_TSIF_ClearPIDTable(file, PID);
				}
				break;
			case FE_DXB_SET_PID_CMD:
				{
					int PID = (int)parg;
					err = TCC_TSIF_SetPID(file, PID);
				}
				break;
			case FE_DXB_REMOVE_PID_CMD:
				{
					int PID= (int)parg;
					err = TCC_TSIF_RemovePID(file, PID);
				}
				break;
			case FE_DXB_SET_PCRPID_CMD:
				{
					int PID= (int)parg;
					err = TCC_TSIF_SetPCRPID(file, PID);
				}
				break;
			case FE_DXB_GET_STC_CMD:
				{
					err = TCC_TSIF_GetSTC(file);
				}
				break;
		}
	}
...
static int dvb_frontend_open(struct inode *inode, struct file *file)
{
	...
	if(fe->ops.tsif_open) {
		fe->ops.tsif_open(inode, file);
	}
	TCC_TSIF_Open(gTSIFDataReadSize / TSIF_PACKETSIZE, TSIF_MAX_PACKETCOUNT, 1, TRUE);
...
static int dvb_frontend_release(struct inode *inode, struct file *file)
{
	...
	if(fe->ops.tsif_close) {
		fe->ops.tsif_close(inode, file);
	}
	TCC_TSIF_Close();
