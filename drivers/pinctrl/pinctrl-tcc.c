				...
				unsigned offset)
{
	...
	for (i = 0; i < pctl->desc->neints; i++)
		if (pctl->eints[i] == offset)
			return pctl->desc->eint_irq_base + i;
...
module_platform_driver(tcc_pinctrl_driver);
