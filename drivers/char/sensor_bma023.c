...
#define BMA023_DEBUG    0
#define BMA023_DEBUG_D  0
...
#define M_RESOLUTIOIN    (256) // BMA023
...
static int bma023_i2c_register(void)
{
    ...
    info.addr = BMA023_I2C_ADDRESS;
    strlcpy(info.type, "tcc-accel-sensor", I2C_NAME_SIZE);

    sensor_dbg(KERN_INFO "%s : bma023_i2c_register\n", __FUNCTION__);
...
static unsigned char bma023_init(void) 
{
//	int comres=0;
    unsigned char data;
    unsigned char chip_id;
...
//	comres += p_bma023->BMA023_BUS_READ_FUNC(p_bma023->dev_addr, BMA023_CHIP_ID__REG, &data, 1);	/* read Chip Id */
    data = SENSOR_READ_DAT(BMA023_CHIP_ID__REG);
	
    chip_id = BMA023_GET_BITSLICE(data, BMA023_CHIP_ID);						/* get bitslice */
    return chip_id;
}
...
// BMA023_RANGE__REG      0x14      (BMA023_RANGE_BWIDTH_REG)
...
// #define BMA023_RANGE__REG				BMA023_RANGE_BWIDTH_REG
    ...
    if (range<3) {	
//	 	comres = p_bma023->BMA023_BUS_READ_FUNC(p_bma023->dev_addr, BMA023_RANGE__REG, &data, 1 );
        data = SENSOR_READ_DAT(BMA023_RANGE__REG);
	 	    data = BMA023_SET_BITSLICE(data, BMA023_RANGE, range);		  	
//    comres += p_bma023->BMA023_BUS_WRITE_FUNC(p_bma023->dev_addr, BMA023_RANGE__REG, &data, 1);
//        SENSOR_SEND_CMD(BMA023_RANGE__REG, &data);
        SENSOR_SEND_CMD(BMA023_RANGE__REG, data);
    }
    return 1;
}
...
/** set BMA023 internal filter bandwidth
   \param bw bandwidth (see bandwidth constants)
   \return result of bus communication function
   \see #define BMA023_BW_25HZ, BMA023_BW_50HZ, BMA023_BW_100HZ, BMA023_BW_190HZ, BMA023_BW_375HZ, BMA023_BW_750HZ, BMA023_BW_0230HZ
   \see bma023_get_bandwidth()
*/
...
// BMA023_BANDWIDTH__REG      0x14           (BMA023_RANGE_BWIDTH_REG)
...
//  	    comres = p_bma023->BMA023_BUS_READ_FUNC(p_bma023->dev_addr, BMA023_BANDWIDTH__REG, &data, 1 );
        data = SENSOR_READ_DAT(BMA023_BANDWIDTH__REG);
	      data = BMA023_SET_BITSLICE(data, BMA023_BANDWIDTH, bw);
//	      comres += p_bma023->BMA023_BUS_WRITE_FUNC(p_bma023->dev_addr, BMA023_BANDWIDTH__REG, &data, 1 );
//        SENSOR_SEND_CMD(BMA023_BANDWIDTH__REG, &data);
        SENSOR_SEND_CMD(BMA023_BANDWIDTH__REG, data);
...
/** X,Y and Z-axis acceleration data readout 
	\param *acc pointer to \ref bma023acc_t structure for x,y,z data readout
...
//	comres = p_bma023->BMA023_BUS_READ_FUNC(p_bma023->dev_addr, BMA023_ACC_X_LSB__REG, &data[0],6);
    SENOSR_READ_DAT_S(BMA023_ACC_X_LSB__REG, &data[0], 6);
	

  
	
//    acc->x = BMA023_GET_BITSLICE(data[0],BMA023_ACC_X_LSB) | (BMA023_GET_BITSLICE(data[1],BMA023_ACC_X_MSB)<<BMA023_ACC_X_LSB__LEN);
...
static int tcc_sensor_open(struct inode *inode, struct file *filp)
{
    ...
    if (sensor_used_count == 0)
    {
        ...
        ret = bma023_i2c_register();   // call register after ( called probe )
        if(ret < 0) 
        {
            sensor_dbg("%s() [Error] Failed register i2c client driver for bma023, return is %d\n", __func__, ret);
            return ret;
        }
        sensor_dbg("%s: post bma023_i2c_register : %x\n", __func__, old_ctrl);
#endif
...
// bma023 -->
        ...
        bma023_set_bandwidth(BMA023_BW_50HZ);		
// bma023 <--
