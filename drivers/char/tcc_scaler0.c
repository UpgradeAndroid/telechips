...
volatile PVIOC_RDMA 	pRDMABase0;
volatile PVIOC_WMIX 	pWIXBase0;
volatile PVIOC_WDMA 	pWDMABase0;
...
static intr_data_t sc0_data;

#define DEVICE_NAME			"scaler0"
#define MAJOR_ID			202
#define MINOR_ID			1
...
static int tccxxx_scaler0_mmap(struct file *file, struct vm_area_struct *vma)
{
	if(range_is_allowed(vma->vm_pgoff, vma->vm_end - vma->vm_start) < 0) {
		printk(KERN_ERR	"scaler0: this address is not allowed. \n");
		return -EAGAIN;
	}
...
//	M2M_Scaler_SW_Reset(M2M_SCALER0);
	...
	if(scale_img->responsetype == SCALER_INTERRUPT)
		MEMSCL_SET_INTERRUPT(M2M_SCALER0, SET_M2MSCL_INT_ENABLE);
	else
		MEMSCL_SET_INTERRUPT(M2M_SCALER0, SET_M2MSCL_INT_DISABLE);

	MEM_SCALER_CTRL(MSC_CTRL_RDY_INT_EN);
	...
	if ((DstImgType == HwMSC_SRC_CFG_YUV422_SEP) || (DstImgType == HwMSC_SRC_CFG_YUV420_SEP))
	{
		...
		MEM_SCALER_DST_CONFIG(MSC_CFG_ALL, 0, 1, 1, interleaved_mode, 0, interleaved_dst, DstImgType);
	}
	else
	{
	...
	MEM_SCALER_CTRL(MSC_CTRL_EN);
	MEM_SCALER_CTRL(MSC_CTRL_DIS);
	MEM_SCALER_CTRL(MSC_CTRL_RDY_INT_EN);
...
#if defined(CONFIG_ARCH_TCC892X)
void tccxxx_scaler0_GetAddress(unsigned char format, unsigned int base_Yaddr, unsigned int src_imgx, unsigned int  src_imgy,
...
char M2M_Scaler0_Ctrl_Detail(SCALER_TYPE *scale_img)
{
	...
	dprintk("TCC892X VIOC Scaler: M2M_Scaler0_Ctrl_Detail(). \n");

	#if(1)
	spin_lock_irq(&(sc0_data.cmd_lock));
	...
	VIOC_API_SCALER_SetPlugIn(VIOC_SC0, scale_img->plugin_path);
	VIOC_API_SCALER_SetUpdate(VIOC_SC0);

	spin_unlock_irq(&(sc0_data.cmd_lock));
	#else
	pSrcBase0 = (unsigned int)scale_img->src_Yaddr;
	pSrcBase1 = (unsigned int)scale_img->src_Uaddr;
	pSrcBase2 = (unsigned int)scale_img->src_Vaddr;
...
//	scaler_ended = check_status_intr = 0;
	...
	if(scale_img->responsetype  == SCALER_POLLING)	{
		ret = wait_event_interruptible_timeout(sc0_data.poll_wq,  sc0_data.block_operating == 0, msecs_to_jiffies(500));
		if(ret <= 0) {
			 sc0_data.block_operating = 0;
...
static unsigned int tccxxx_scaler0_poll(struct file *filp, poll_table *wait)
{
	...
	if(msc_data == NULL) 	return -EFAULT;

	poll_wait(filp, &(msc_data->poll_wq), wait);

	spin_lock_irq(&(msc_data->poll_lock));

	if(msc_data->block_operating == 0) 	ret = (POLLIN|POLLRDNORM);
...
// interrupt handler
static irqreturn_t tccxxx_scaler0_handler(int irq, void *client_data/*, struct pt_regs *reg*/)
{  	
	unsigned long IFlag;
	intr_data_t *msc_data = client_data;
	
	#if defined(CONFIG_ARCH_TCC892X)
	if(pWDMABase0->uIRQSTS.nREG & VIOC_WDMA_IREQ_EOFR_MASK) {
	...
	if(msc_data->block_operating >= 1) 	msc_data->block_operating = 0;
		
	wake_up_interruptible(&(msc_data->poll_wq));

	if(msc_data->block_waiting) 	wake_up_interruptible(&msc_data->cmd_wq);
...
long tccxxx_scaler0_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	dprintk("scaler0  ::cmd:0x%x block_operating(%d) - block_waiting(%d) - cmd_count(%d) - poll_count(%d)!!!\n", 	\
	...
	switch(cmd) {
		...
		case TCC_SCALER_IOCTRL_KERENL:
			...
			if(ret >= 0) {
				...
				ret = M2M_Scaler0_Ctrl_Detail(&scaler_v);
		...
		case TCC_SCALER_VIOC_PLUGOUT:
			VIOC_RDMA_SetImageIntl(pRDMABase0, 0);
...
{
	int ret = 0;
	dprintk("scaler0_release In!! %d'th, block(%d/%d), cmd(%d), irq(%d)  \n", sc0_data.dev_opened, sc0_data.block_operating, \
																sc0_data.block_waiting, sc0_data.cmd_count, sc0_data.irq_reged);

	if(sc0_data.dev_opened > 0) 	sc0_data.dev_opened--;
	if(sc0_data.dev_opened == 0) {
		if(sc0_data.block_operating) {
			ret = wait_event_interruptible_timeout(sc0_data.cmd_wq, sc0_data.block_operating == 0, msecs_to_jiffies(200));
		}

		if(ret <= 0) {
 			printk("[%d]: scaler0 timed_out block_operation:%d!! cmd_count:%d \n", ret, sc0_data.block_waiting, sc0_data.cmd_count);
		}

		#if defined(CONFIG_ARCH_TCC892X)
		if(sc0_data.irq_reged) {
			//free_irq(INT_VIOC_WD3, &sc0_data);
			sc0_data.irq_reged = 0;
		}
		VIOC_SC_SetSWReset(VIOC_SC0, 0xFF/*RDMA*/, 0xFF/*WDMA*/);
		#else // CONFIG_ARCH_TCC892X
		...
		sc0_data.block_operating = sc0_data.block_waiting = 0;
		sc0_data.poll_count = sc0_data.cmd_count = 0;
	}
...
{	
	...
	if(!sc0_data.irq_reged) {
		...
		sc0_data.irq_reged = 1;
	}

	sc0_data.dev_opened++;
	filp->private_data = &sc0_data;
...
static struct file_operations tcc_scaler0_fops = 
{
	.owner 			= THIS_MODULE,
...
static struct class *scaler0_class;

int __init 
tccxxx_scaler0_init(void)
{
	printk(banner);
	
	register_chrdev(MAJOR_ID, DEVICE_NAME, &tcc_scaler0_fops);
	scaler0_class = class_create(THIS_MODULE, DEVICE_NAME);
	device_create(scaler0_class, NULL, MKDEV(MAJOR_ID, MINOR_ID), NULL, DEVICE_NAME);

	memset(&sc0_data, 0, sizeof(intr_data_t));

	spin_lock_init(&(sc0_data.poll_lock));
	spin_lock_init(&(sc0_data.cmd_lock));

	mutex_init(&(sc0_data.io_mutex));
	
	init_waitqueue_head(&(sc0_data.poll_wq));
	init_waitqueue_head(&(sc0_data.cmd_wq));
...
MODULE_DESCRIPTION("TCC Scaler-0 Driver");
MODULE_LICENSE("GPL");


arch_initcall(tccxxx_scaler0_init);
