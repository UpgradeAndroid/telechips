...
//      }
	ret =
	    i2c_write_bytes(ts->client, pdata->config_info,
			    pdata->config_info_len);
	pr_info("ret = %d\n", ret);
	if (ret < 0)
...
static ssize_t goodix_read(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	int i = 0;
	...
	if (!strcmp(attr->attr.name, "ctpconfig")) {
		config_data[0] = 0x30;
		i2c_read_bytes(ts->client, config_data,
			       sizeof(config_data));
		memcpy(buf, config_data, sizeof(config_data));
...
static ssize_t goodix_write(struct device *dev, struct device_attribute *attr,
	...
	if (!strcmp(attr->attr.name, "ctpconfig")) {
		...
		for (i = 0; i < 100; i++) {
			if (sscanf(buf, "%x", &val) == 1) {
				printk("%x\n", config_data[i] = (unsigned char)val);
				buf += 5;
			} else
				break;
		}
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	if (gpio_get_value(SHUTDOWN_PORT)) {
		printk(KERN_ALERT
		       "Guitar stop working.The data is invalid. \n");
		goto NO_ACTION;
	}
	...
	while (1) {
		...
		if (ret > 0)
			...
			dev_err(&(ts->client->dev),
				"%dth I2C transfer error. Number:%d\n ", retry,
				ret);
			msleep(20);
			...
			if (++retry > 3) {
				dev_info(&(ts->client->dev),
					 "Because of transfer error, Guitar's driver stop working.\n");
				goto XFER_ERROR;
			}
		}
	}
	...
	goodix_dbg(dbg_en, "point_data[1]=%d\n", point_data[1]);
	...
	for (position = 0; (finger != 0) && (position < MAX_FINGER_NUM);
	     position++) {
...
BIT_NO_CHANGE:
	for (count = 0; count < finger_list.length; count++) {
		...
		goodix_dbg(dbg_en, "Count:%d Index:%d X:%d Y:%d\n", count,
			   finger_list.pointer[count].num, pos[count][0],
			   pos[count][1]);
	}
...
*/
	...
	if (ts->input_dev == NULL) {
		ret = -ENOMEM;
		dev_dbg(&client->dev,
			" In File:%s Function:%s Failed to allocate input device\n",
			__FILE__, __func__);
	...
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, pdata->xmax,
			     0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, pdata->ymax,
			     0, 0);
#endif
	...
	if (client->irq) {
		...
		ret = request_irq(client->irq, goodix_ts_irq_handler,
				  pdata->
				  irq_edge ? IRQ_TYPE_EDGE_FALLING :
