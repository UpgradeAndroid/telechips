...
void tcc_sc_init_port(void)
{
	// RESET
	#if defined(CONFIG_MACH_TCC8800)
	gpio_request(GPIO_CAS_RST,"sc_rst");

	tcc_gpio_config(TCC_GPD(13), GPIO_FN(0));
	tcc_gpio_config(TCC_GPEXT2(7), GPIO_FN(0));
	tcc_gpio_config(TCC_GPEXT2(6), GPIO_FN(0));
	...
	tcc_gpio_config(TCC_GPD(13), GPIO_FN(0));	// CAS_DATA
	tcc_gpio_config(TCC_GPD(15), GPIO_FN(0));	// CAS_RST#
	/*******************************************************************/
	// Originaly "GPIO_B(28)" is "TCC8801F_STB" Test Pin . 
	// "GPIO_B(28)" use to in test.
	tcc_gpio_config(TCC_GPB(28), GPIO_FN(0));	// CAS_GP(SC_CDMVCC#)
...
void tcc_sc_init_buffer(unsigned uCH, unsigned uTxRx)
{
...
void tcc_sc_rx_push_char(unsigned uCH, char cChar)
{
...
unsigned tcc_sc_rx_pop_char(unsigned uCH)
{
	sDRV_UART_BUF 	*pUartBuf = &gDRV_UART_BUF[uCH];
	unsigned 		uBufData;
...
void tcc_sc_ctrl_txport(unsigned uCH, unsigned uEnable)
{
...
unsigned tcc_sc_receive_byte(unsigned int uCH)
{
	UART *pHwUART = (volatile PUART)tcc_p2v(SC_UART_ADDR(uCH));
	unsigned uByte;
...
void tcc_sc_send_byte(unsigned int uCH, unsigned uChar)
{
	UART *pHwUART = (volatile PUART)tcc_p2v(SC_UART_ADDR(uCH));
...
void tcc_sc_get_atr(unsigned char *pATRData, unsigned *pATRLength)
{
	...
	//Init Tx/Rx buffer
	tcc_sc_init_buffer(uCH, DRV_UART_BUFFER_TXRX);

	tcc_sc_make_reset(1);	// Reset HIGH

	//Disable SmartCard TX
	tcc_sc_ctrl_txport(uCH, DISABLE);
	
	//Get ATR Response
	while (1)
	{
		uData = tcc_sc_rx_pop_char(uCH);
		if(uData < 0x100)
		{
			*pucBuffer++ = uData;
			uLength++;
...
int tcc_sc_parse_atr(unsigned char *pATRData, unsigned uATRLength, sTCC_SC_ATR_CHARS *pstATRParse)
{
	int i;
	int iProtocol = 0;
...
int tcc_sc_set_factor(sDRV_UART *pstDrvSc, unsigned char ucF, unsigned char ucD)
{
...
void tcc_sc_interrupt_enable(unsigned uCH, unsigned uTxEn, unsigned uRxEn)
{
	UART *pHwUART;
...
irqreturn_t tcc_sc_interrupt_handler(int irq, void *dev_id)
{
	unsigned char ucRcvData;
	unsigned char ucCH = gDRV_UART_SC.CHANNEL;
	
	//Get a Received Byte
	ucRcvData = tcc_sc_receive_byte(ucCH);
	
	//Push Data to UART Buffer
	tcc_sc_rx_push_char(ucCH, ucRcvData);

    return IRQ_HANDLED;
}

void tcc_sc_clock_ctrl(sDRV_UART *pstDrvSc, unsigned uEnable)
{
...
int tcc_sc_config(sDRV_UART *pstDrvSc)
{
	...
	tcc_sc_clock_ctrl(pstDrvSc, ENABLE);
...
int tcc_sc_enable(sDRV_UART *pstDrvSc)
{
	...
	*pCHSEL = uCHSEL;
	...
	request_irq(INT_UART0+uUartCH, tcc_sc_interrupt_handler, IRQF_SHARED, "tcc_sc", &iIrqData);
...
int tcc_sc_disable(sDRV_UART *pstDrvSc)
{
	unsigned char uUartCH;
	UART *pHwUART;
	...
	tcc_sc_clock_ctrl(pstDrvSc, DISABLE);
...
int tcc_sc_reset(unsigned char *pATR, unsigned *pATRLength)
{
	...
	tcc_sc_set_factor(&gDRV_UART_SC, 1, 1);

	for(iRetry = 3; iRetry > 0; iRetry--)
	{
		tcc_sc_get_atr(pATR, pATRLength);
		if(*pATRLength > 0)
		{
			iRet = 0;
			break;
	...
	if(iRet == 0)
	{
		if((iProtocol = tcc_sc_parse_atr(pATR, *pATRLength, &stATRChar)) < 0)
		{
		...
		else
		{
			...
			tcc_sc_set_factor(&gDRV_UART_SC, g_SCParams.ucF, g_SCParams.ucD);
...
void tcc_sc_send_data(char *pData, unsigned uLength)
{
	UART 			*pHwUART;
	...
	if(uLength > 0)
	{
		...
		while(uSendLength)
		{
			...
			tcc_sc_send_byte(uCH, *pucBuffer++);			
			uSendLength--;
		}
...
unsigned tcc_sc_receive_data(char *pData, unsigned uLength)
{
	unsigned char 	*pucBuffer 	= pData;
...
unsigned tcc_sc_receive_data_len(char *pData, unsigned *pLength)
{
	unsigned char 	*pucBuffer 	= pData;
	unsigned 		uRcvLength 	= 0;
...
int	tcc_sc_send_receive(char *pSend, unsigned uSendLength, char *pRcv, unsigned uRcvLength)
{
	int	ret	= -1;
	...
	if(pSend && uSendLength)
		tcc_sc_send_data(pSend, uSendLength);
	...
	if(pRcv!=NULL && uRcvLength!=0)
	{
		...
		{
			ret	= tcc_sc_receive_data(pRcv, uRcvLength);
			if(ret <= 0)
				ret	= -1;
		}
		else
...
int	tcc_sc_send_receive_len(char *pSend, unsigned uSendLength, char *pRcv, unsigned *pRcvLength)
{
	...
	{
		iLength	= tcc_sc_receive_data_len(pRcv, pRcvLength);
		if(iLength > 0)
			iRet = iLength;
	}
...
static int tcc_sc_ioctl(struct inode *inode, struct file *file, unsigned int cmd, void *arg)
{
	//dprintk("%s, cmd=%d\n", __func__, cmd);
