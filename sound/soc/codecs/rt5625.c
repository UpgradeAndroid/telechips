...
#define RT5625_VERSION "0.03 alsa 1.0.23"
...
static const char *rt5625_aec_path_sel[] = {"aec func disable","aec pcm in/out",
	...
	if(bEnableVoicePath)
	 {
			...
			rt5625_write_mask(codec,RT5625_VOICE_DAC_OUT_VOL,0,M_V_DAC_TO_MONO_MIXER);
		...
		switch(mode)		
		{
			...
			case PCM_MASTER_MODE_B:	//8kHz sampling rate,16 bits PCM and Master mode,PCM mode is B,Clock from PLL OUT
				...
				//rt5625_write(codec,RT5625_EXTEND_SDP_CTRL,0x8083); //Cary
				rt5625_write(codec,RT5625_EXTEND_SDP_CTRL,0x8082);
				...
				//the register 0x64 value's should is 0x5524
				//rt5625_write(codec,RT5625_VOICE_DAC_PCMCLK_CTRL1,0x5524);
				rt5625_write(codec,RT5625_VOICE_DAC_PCMCLK_CTRL1,0x5610);
...
static int rt5625_reinit(struct snd_soc_codec *codec);
//***********************************************************************************************
...
static int set_vodsp_aec_path(struct snd_soc_codec *codec, unsigned int mode)
{
		...
		switch(mode)
		{
			...
			case PCM_IN_PCM_OUT:
				...
				#if 0
				rt5625_write_mask(codec, 0x26,0x0300,0x0300);
				rt5625_write(codec, RT5625_VODSP_PDM_CTL,0x0000);
				rt5625_write(codec, RT5625_DAC_ADC_VODAC_FUN_SEL,0x0010);
				rt5625_write(codec, RT5625_VODSP_CTL,VODSP_LRCK_SEL_16K);						
				rt5625_write_mask(codec, 0x26,0x0000,0x0300);
				#endif
				#endif
				#if 0
				...
				#else
				rt5625_write_mask(codec, 0x3a,0x088f,0x088f);//power on related bit
				rt5625_write_mask(codec, 0x3c,0xa4c7,0xa4c7);//power on related bit
				rt5625_write_mask(codec, 0x3e,0x4330,0xf330);//power on related bit
						
				rt5625_write(codec, 0x10, 0xee0f);//mute DAC to hpmixer
				rt5625_write(codec, 0x08, 0xe800);//set phone to differential mode
				//rt5625_write(codec, 0x22, 0x0400);//Mic boost +20db
				rt5625_write(codec, 0x12, 0xCBD3);//ADC_Mixer_R boost 10.5 db
				rt5625_write(codec, 0x14, 0x7777);//phone->ADCMixer_R
				rt5625_write(codec, 0x18, 0xc010);//VoDAC to AuxMixer,12db
				rt5625_write(codec, 0x1c, 0x00d0);//aux source from auxmixer
				//rt5625_write(codec, 0x1a, 0x0000);//speaker source from speakermixer
			
				rt5625_write(codec,0x40,0x8c0a);
				rt5625_write(codec,0x44,0x3b25);
				
							
				rt5625_write_mask(codec, 0x02,0x8080,0x8080);	//mute speaker 
				rt5625_write_mask(codec, 0x04,0x8080,0x8080);	//mute hp
				rt5625_write_mask(codec, 0x06,0x0000,0x8080);	//unmute aux
				#endif
			...
			case ANALOG_IN_ANALOG_OUT:	
				...
				rt5625_write(codec, 0x12, 0xCBCB);//ADC_Mixer_R boost 10.5 db
			...
			default:
				...
				rt5625_write(codec, 0x08,0xc800);//mute auxout
				...
				rt5625_write(codec, 0x1c, 0x0308);//all output from HPmixer by default
				rt5625_write(codec, 0x1e, 0x2007);
				rt5625_write(codec, 0x10, 0xee03);//DAC to HPmixer by default
				rt5625_write(codec, 0x18, 0xe010);//mute VoDAC to mixer by default
				rt5625_write(codec, 0x64, 0x3110);
				rt5625_write(codec, 0x44, 0x0000);
				rt5625_write(codec, 0x48, 0x0029);
				rt5625_write(codec, 0x40, 0x0C0A);
				rt5625_write(codec, 0x36, 0x0000);
				rt5625_write_mask(codec, 0x26,0x0000,0x0300);
				...
				//rt5625_write_mask(codec, 0x3a,0xc84f,0xc84f);//power down related bit
				//rt5625_write_mask(codec, 0x3c,0x0000,0x07cf);//power down related bit
				rt5625_write(codec, 0x3e, 0x8000);
				rt5625_write(codec, 0x3a, 0xc84f);
...
#define TEST_REG 1
#if TEST_REG
static int print_reg(struct snd_soc_codec *codec)
{
	int i;
	for(i=0;i<=0x6e; i+=2) {
		printk("{0x%x, 0x%x},", i, rt5625_read(codec, i));
		if(i%8==0)
			printk("\n");
	}
}
...
static int spk_pga_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *k, int event)
 {
	...
	printk(KERN_INFO "enter %s, w->name=%s\n", __func__, w->name);
...
static const struct snd_soc_dapm_route audio_map[] = {
		...
		{"AUXOUT Mux", "Mono Mixer", "MoNo Mixer"},  //Modify by Cary. chen at 2011.08.10
...
//For test
	...
	int reg_val=0;
	for(i=0; i < 8; i+=2) {
		reg_val = rt5625_read(codec, i);
		printk("reg_val[%d]=0x%x\n", i, reg_val);
	}

	rt5625_write(codec, 0x02, 0x5a1a);
	reg_val= rt5625_read(codec, 0x02);
	printk("reg_val[2]=0x%x\n", reg_val);
#endif	
...
static int rt5625_probe(struct platform_device *pdev)
{
	...
	struct snd_soc_codec *codec;
	struct rt5625_priv *rt5625;//=snd_soc_codec_get_drvdata(codec); //Access null point, Wrong!!!!
	int ret;

	pr_info("RT5625 Audio Codec %s\n", RT5625_VERSION);
	...
	snd_soc_codec_set_drvdata(codec, (void *)rt5625);
